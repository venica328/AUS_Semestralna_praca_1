...
namespace structures {
	...
	Vector::Vector(const Vector& other) :
		Vector(other.size_)
	{
		//pouzili sme memcpy rychlejsiu operaciu a nemusime sa bat prekrytia
		//lebo kazdy vektor ma svoju pamat
	...
	void Vector::copy(const Vector& src, const int srcStartIndex, Vector& dest, const int destStartIndex, const int length)
	{
		if (length != 0)
		{
			DSRoutines::rangeCheckExcept(srcStartIndex, src.size_, "Invalid start index in Vector src!");
			DSRoutines::rangeCheckExcept(destStartIndex, dest.size_, "Invalid start index in Vector dest!");
